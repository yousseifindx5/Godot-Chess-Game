...
const BLACK_BISHOP = preload("res://Assets/Graphics/black_bishop.png")
const BLACK_KING = preload("res://Assets/Graphics/black_king.png")
const BLACK_KNIGHT = preload("res://Assets/Graphics/black_knight.png")
const BLACK_PAWN = preload("res://Assets/Graphics/black_pawn.png")
const BLACK_QUEEN = preload("res://Assets/Graphics/black_queen.png")
const BLACK_ROOK = preload("res://Assets/Graphics/black_rook.png")
const WHITE_BISHOP = preload("res://Assets/Graphics/white_bishop.png")
const WHITE_KING = preload("res://Assets/Graphics/white_king.png")
const WHITE_KNIGHT = preload("res://Assets/Graphics/white_knight.png")
const WHITE_PAWN = preload("res://Assets/Graphics/white_pawn.png")
const WHITE_QUEEN = preload("res://Assets/Graphics/white_queen.png")
const WHITE_ROOK = preload("res://Assets/Graphics/white_rook.png")
const TURN_WHITE = preload("res://Assets/Graphics/turn-white.png")
const TURN_BLACK = preload("res://Assets/Graphics/turn-black.png")
...
func _ready():
	if GameManager.game_in_progress && !GameManager.saved_board.is_empty():
		board = GameManager.saved_board.duplicate(true)
		white = GameManager.saved_white_turn
	else:
...
func _input(event):
	...
	if event is InputEventMouseButton && event.pressed && promotion_square == null:
		if event.button_index == MOUSE_BUTTON_LEFT:
			...
			var var2 = int((-get_global_mouse_position().y) / CELL_WIDTH)
			if var1 < 0 || var1 >= BOARD_SIZE || var2 < 0 || var2 >= BOARD_SIZE: return
	...
	for i in moves:
		if i.x == var2 && i.y == var1:
			...
			if !is_promotion:
				...
				if GameManager.game_mode == "computer" && !white:
	...
	if !is_promotion:
		if (selected_piece.x != var2 || selected_piece.y != var1) && (white && board[var2][var1] > 0 || !white && board[var2][var1] < 0):
			...
			if white && is_in_check(white_king_pos):
				...
				GameManager.saved_board = []
				await get_tree().create_timer(1.0).timeout
				get_tree().change_scene_to_file("res://Scenes/Black wins.tscn")
			elif !white && is_in_check(black_king_pos):
				GameManager.game_in_progress = false
				GameManager.saved_board = []
				await get_tree().create_timer(1.0).timeout
				get_tree().change_scene_to_file("res://Scenes/White wins.tscn")
			else:
				GameManager.game_in_progress = false
				GameManager.saved_board = []
				await get_tree().create_timer(1.0).timeout
...
func make_computer_move():
	var best_move = ChessAI.get_best_move(board, GameManager.difficulty, self)
	if best_move.is_empty(): return
	selected_piece = best_move.from
	moves = get_moves(selected_piece)
	var piece = board[best_move.from.x][best_move.from.y]
	var is_pawn_promotion = (piece == -1 && best_move.to.x == 0) || (piece == 1 && best_move.to.x == 7)
	if is_pawn_promotion:
		board[best_move.to.x][best_move.to.y] = 5 if white else -5
		board[best_move.from.x][best_move.from.y] = 0
		white = !white
		AudioManager.play_move()
		display_board()
		return
	set_move(best_move.to.x, best_move.to.y)
...
func _on_options_pressed():
	...
	GameManager.came_from_game = true
	GameManager.saved_board = board.duplicate(true)
	GameManager.saved_white_turn = white
...
func get_pawn_moves(piece_position: Vector2):
	...
	if is_first_move && is_valid_position(pos) && is_empty(pos) && is_empty(piece_position + direction):
		board[pos.x][pos.y] = 1 if white else -1
		board[piece_position.x][piece_position.y] = 0
		if white && !is_in_check(white_king_pos) || !white && !is_in_check(black_king_pos): _moves.append(pos)
		board[pos.x][pos.y] = 0
...
func is_in_check(king_pos: Vector2):
	...
	for i in pawn_attacks:
		if is_valid_position(i):
			if (is_white && board[i.x][i.y] == -1 || !is_white && board[i.x][i.y] == 1): return true

	for i in directions:
		var pos = king_pos + i
		if is_valid_position(pos):
			if is_white && board[pos.x][pos.y] == -6 || !is_white && board[pos.x][pos.y] == 6: return true
	...
	for i in directions:
		...
		while is_valid_position(pos):
			if !is_empty(pos):
				...
				if (i.x == 0 || i.y == 0) && (is_white && piece in [-4, -5] || !is_white && piece in [4, 5]):
					return true
				elif (i.x != 0 && i.y != 0) && (is_white && piece in [-3, -5] || !is_white && piece in [3, 5]):
					return true
				break
			pos += i
	...
	for i in knight_directions:
		var pos = king_pos + i
		if is_valid_position(pos):
			if is_white && board[pos.x][pos.y] == -2 || !is_white && board[pos.x][pos.y] == 2:
				return true
